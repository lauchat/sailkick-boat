...
function createSync (app, options) {
  ...
  function start () {
    ...
    const handleDelta = (delta) => {
      try {
        const lines = deltaToLines(ownNotificationsStripped(delta), { context: selfContext })
        if (lines.length) state.batch.push(...lines)
        if (state.batch.length >= state.batchSize) flush()
      } catch (e) { log('delta error: ' + e.message) }
    }

    // Our OWN alarm notifications go up as the `alerts` measurement (lib/alerts/relay.js),
    // which is a schema built for the purpose. Letting them ALSO travel the generic path
    // would write each one a second time as flattened notifications.* rows — the state,
    // every element of the method array, and whatever ack/silence bookkeeping the server
    // hangs off the path — describing the same event twice, in a worse shape, in a
    // namespace that means something else. Only ours: every other plugin's notifications
    // (the Victron monitors, the server's own) keep flowing exactly as before.
    const ownNotificationsStripped = (delta) => {
      const own = options.pluginId
      if (!own || !delta || !Array.isArray(delta.updates)) return delta
      let touched = false
      const updates = delta.updates.map((u) => {
        if (!u || !Array.isArray(u.values)) return u
        const src = typeof u.$source === 'string' ? u.$source : (u.source && u.source.label)
        if (src !== own) return u
        const values = u.values.filter((pv) => !(pv && typeof pv.path === 'string' && pv.path.startsWith('notifications.')))
        if (values.length === u.values.length) return u
        touched = true
        return { ...u, values }
      })
      return touched ? { ...delta, updates } : delta
    }
  ...
  // Put caller-built line protocol through the SAME spool as telemetry. lib/alerts uses
  // this for alarm transitions: they then inherit the store-and-forward guarantees rather
  // than needing a second, weaker delivery path — an alarm raised mid-ocean arrives when
  // the link does, in order, instead of being lost the moment the POST fails.
  //
  // Returns false when sync is not configured (no account yet), so the caller can say
  // "alarms are local-only" rather than believing they were sent.
  // NB the name: this must NOT be called writeLines. That is the module-scope import from
  // ./influxWrite that pump() uses to POST a batch, and a same-named function here shadows
  // it inside createSync — pump() then "uploads" by calling this, gets false, reads it as a
  // rejected batch and QUARANTINES the file. Every buffering test failed at once, which is
  // the only reason it was caught. (Same bug as lib/cloud's `request` shadowing, v0.23.4.)
  function enqueueLines (lines) {
    if (!state || state.stopped || !Array.isArray(lines) || !lines.length) return false
    state.batch.push(...lines)
    // Alarms are rare and urgent: flush immediately instead of waiting for the batch to
    // fill, which at a boat's data rate could be a minute away.
    if (state.flushNow) state.flushNow()
    return true
  }
  ...
  return { start, stop, status, pending, writeLines: enqueueLines }
}
