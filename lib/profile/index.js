...
// Item sections, as the cloud router defines them (server/routes/profile.js). 'alerts'
// is boat-side first: alarm rules belong where they are evaluated, and this copy is
// deliberately not synced, so a rule edited from ashore cannot silently change what the
// boat alarms on mid-passage.
const SECTIONS = ['polars', 'routes', 'alerts']
const MAX_BODY_BYTES = 4 * 1024 * 1024 // a polar table is the big one; well under this
const emptyProfile = () => ({ polars: [], activePolar: null, routes: [], alerts: [], settings: {} })
...
function createProfile (app, options = {}) {
  ...
  // Patch one item in place, through the SAME serialized queue as the HTTP writes, so a
  // caller inside the process (lib/alerts dropping an anchor datum) cannot interleave with
  // a save from the app and lose one of them. Returns the patched item, or null if the id
  // is unknown.
  async function patchItem (section, id, patch) {
    if (!SECTIONS.includes(section)) throw new Error(`unknown profile section "${section}"`)
    let out = null
    await update((p) => {
      const arr = Array.isArray(p[section]) ? p[section] : (p[section] = [])
      const i = arr.findIndex((x) => x && x.id === id)
      if (i < 0) return p
  ...
  return { start, stop, status, available, handles, handle, patchItem, _load: load, _file: () => file }
}
