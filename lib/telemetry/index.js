...
//
// The vendored mapper's resolveHeadingDeg() does the opposite, for a real reason: some
// boats publish a stale or static headingTrue — the app found one sitting at exactly 151°
// while its NMEA2000 compass read true ~293°. Preferring the published value blindly
// would adopt that.
//
// But we hold BOTH numbers, so the pathology is detectable rather than something to guard
// against by avoidance. Measured on this boat they agree to 0.52° (published 49.00°,
// derived 49.52°), which is what a healthy installation looks like. So: use the published
// value while it corroborates the compass, and fall back to the derived one — saying so —
// when it does not. A frozen headingTrue diverges by tens of degrees within a turn, so the
// threshold does not need to be tight.
