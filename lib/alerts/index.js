...
function createAlerts (app, options = {}) {
  ...
  function tick () {
    ...
    for (const ev of engine.tick(now)) {
      ...
      if (ev.kind === 'feed-stale') {
        ...
        if (!jumpy) { jumpWarned = false; log('position source settled'); continue }
  ...
  function stop () {
    ...
    jumpy = false; jumpWarned = false; invalid = 0; invalidWarned.clear()
  }
  ...
  function status () {
    ...
    const on = rules.filter((r) => r.enabled !== false).length
    if (jumpy) {
      return 'alerts: position is JUMPING between sources — pin navigation.position or the anchor alarm cannot fire'
    }
    ...
    return `alerts: ${on} rule(s), none raised${raisedCount ? `; ${raisedCount} since start` : ''}${bad}`
  }
