...
const fs = require('fs')
const { createAlertEngine, STALE_SEC } = require('./alerts')
...
// A position that implies this speed did not come from a boat. Measured on this vessel:
// with two sources publishing navigation.position and neither pinned, the second was a
// median 2.3 km from the first with fix-to-fix jumps of up to 22.7 km — while the good
// receiver, at rest for 90 minutes, scattered a median 1.88 m from its centroid (max
// 2.91 m). That contention does not produce a false alarm; it produces something worse.
// The far source reads as "outside the circle" and the good one as "inside", and since a
// raise needs the condition to hold CONTINUOUSLY, the alternation resets the hold every
// other sample and the anchor alarm can never fire at all. Silently. So: notice the jumps
// and say so, because an owner asleep behind a dead alarm is the one outcome this feature
// must not produce. 60 kt is far above any sailing yacht and far below a source conflict.
const IMPLAUSIBLE_KT = 60
const MIN_JUMP_M = 100 // ignore scatter entirely; this is about kilometre-scale disagreement
...
function createAlerts (app, options = {}) {
  ...
  let lastFix = null // {t, lat, lon} — for the position-jump check below
  let jumps = 0
  ...
  function readRules () {
    try {
      ...
      return Array.isArray(p && p.alerts) ? p.alerts.filter((r) => r && r.id && r.kind) : []
    } catch { return [] }
  }
  ...
  // Two sources fighting over navigation.position look exactly like a boat teleporting.
  // Only worth saying when an anchor rule is enabled — it is the rule the disagreement
  // disables, and on a boat under way an occasional jump is a curiosity, not a danger.
  function checkPositionJump (s, at) {
    if (!Number.isFinite(s.lat) || !Number.isFinite(s.lon)) return
    const prev = lastFix
    lastFix = { t: at, lat: s.lat, lon: s.lon }
    if (!prev) return
    const dt = (at - prev.t) / 1000
    if (dt <= 0.5) return
    const m = greatCircleKm(prev.lat, prev.lon, s.lat, s.lon) * 1000
    if (m < MIN_JUMP_M || (m / dt) * MS_TO_KT < IMPLAUSIBLE_KT) return
    jumps++
    const anchored = rules.some((r) => r.kind === 'anchor-drift' && r.enabled !== false)
    if (!anchored || jumpWarned) return
    jumpWarned = true
    warn(`position jumped ${Math.round(m)} m in ${dt.toFixed(1)}s — that is ${Math.round((m / dt) * MS_TO_KT)} kt, so more than one source is publishing navigation.position. ` +
      'An anchor alarm CANNOT be relied on until one is pinned: the sources take turns being outside the circle, which resets the hold time and means the alarm never fires. ' +
  ...
  function tick () {
    ...
    for (const ev of engine.tick(now)) {
      // The feed dying is a plugin-health problem, not a navigational alarm: it goes to
      // the status line and the log rather than onto the bus, where it would ring the same
      // bell as a dragging anchor.
      stale = ev.transition === 'raised'
      if (stale) warn(`telemetry feed stale — no fresh data for ${STALE_SEC}s; alert rules are NOT being evaluated`)
      else log('telemetry feed recovered')
    }
  }
  ...
  function stop () {
    ...
    lastFix = null; jumps = 0; jumpWarned = false
  }

  // Drop-anchor: set the datum without editing the rule. Wired to an app action later;
  // exposed now so the engine's setAnchor is reachable from the plugin.
  ...
  function status () {
    ...
    const on = rules.filter((r) => r.enabled !== false).length
    if (jumps && rules.some((r) => r.kind === 'anchor-drift' && r.enabled !== false)) {
      return `alerts: position is JUMPING between sources (${jumps}x) — pin navigation.position or the anchor alarm cannot fire`
    }
    if (stale) return `alerts: FEED STALE — ${on} rule(s) not being evaluated`
    if (!on) return 'alerts: no rules'
    const active = engine.active.filter((id) => id !== '__feed__')
    if (active.length) return `alerts: ${active.length} RAISED (${active.join(', ')}) of ${on} rule(s)`
    return `alerts: ${on} rule(s), none raised${raisedCount ? `; ${raisedCount} since start` : ''}`
  }
