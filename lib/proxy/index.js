...
function createProxy (app, options) {
  ...
  async function serveMirror (req, res) {
    ...
    // Drop anchor, from the boat's own fix. POST-only, and never proxied upstream: the
    // cloud has no rule engine and could not answer it anyway. The datum is written into
    // the rule (durable) and armed on the live engine in one step — see lib/alerts.
    if (req.method === 'POST' && cfg.alerts && cfg.alerts.available() &&
        req.url.split('?')[0] === '/api/alerts/anchor') {
      return readJsonBody(req).then((body) => cfg.alerts.dropAnchor(body || {}))
        .then((r) => {
          res.statusCode = r.ok ? 200 : (r.status || 400)
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify(r.ok ? r : { ok: false, code: r.code, message: r.message }))
        })
        .catch((e) => { res.statusCode = 500; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ ok: false, code: 'alerts-error', message: e.message })) })
    }
  ...
  // Signal K's router runs bodyParser.json() before us; the mirror does not. Handle both,
  // or a POST arriving through the plugin router hangs for ever waiting for 'data' events
  // that already fired — the bug that once left the Sync page on "Signing in…".
  function readJsonBody (req) {
    ...
    return new Promise((resolve) => {
      let raw = ''
      req.on('data', (c) => { raw += c; if (raw.length > 65536) { raw = ''; req.destroy() } })
    ...
    try {
      ...
      try {
        ...
        if (j && typeof j === 'object') {
          ...
          // The app's Alerts pane shows an amber "these rules are not being evaluated"
