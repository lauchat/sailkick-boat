...
function createHistory (app, options) {
  ...
  function start () {
    ...
    if (cfg.url && cfg.token && cfg.bucket) {
      // Full history from a local InfluxDB (the boat's bandg).
      ...
      // Persist to the plugin data dir (append-log) so it survives restarts, unless
      // ringPersist is off. Window/rate configurable (rate auto-coarsens for big windows).
      const persist = options.ringPersist !== false && app.getDataDirPath
      const persistFile = persist ? path.join(app.getDataDirPath(), 'history-ring.jsonl') : null
