...
function createHistory (app, options) {
  ...
  function start () {
    ...
    if (cfg.url && cfg.token && cfg.bucket) {
      ...
      provider = new RingHistoryProvider({ source: options.ringSource, windowSec: options.ringWindowSec, sampleSec: options.ringSampleSec })
      mode = 'ring'
      log('serving /api/history from a DB-less 1h ring (no local InfluxDB)')
    } else {
  ...
  function status () {
    ...
    return mode === 'ring' ? 'history: ring 1h' : `history: ${cfg.bucket}@local`
  }
