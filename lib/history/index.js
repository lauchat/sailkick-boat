...
// /api/history/track from the boat's LOCAL InfluxDB (the bandg bucket), so the
// app's Trends panel + track work offline with the full local dataset.
//
// This is the boat-edge counterpart of the app's server/history/influx-provider.js:
// the Flux queries and channel map are ported VERBATIM (the boat's bandg schema —
...
function createHistory (app, options) {
  ...
  function start () {
    const url = (options.influxUrl || 'http://127.0.0.1:8086').replace(/\/+$/, '')
    cfg = {
      url,
      org: options.org || 'addiction',
      bucket: options.bucket || 'bandg',
      token: options.token || '',
      timeoutMs: options.requestTimeoutMs || 15000
    }
    log(available()
      ? `serving /api/history from ${cfg.url} bucket "${cfg.bucket}"`
      : 'configured off (no local Influx URL/token/bucket) — /api/history falls through to the mirror')
  }
  ...
  function available () { return !!(cfg && cfg.url && cfg.token && cfg.bucket) }

  function status () {
    if (!available()) return 'history: off'
    return `history: ${cfg.bucket}@local`
  }
  ...
  async function getSeries ({ windowSec, everySec, signal }) {
  ...
  async function getTrack ({ windowSec, everySec = 30, signal }) {
    const flux = `from(bucket: "${cfg.bucket}")
  |> range(start: -${windowSec}s)
  ...
  async function handleSeries (req, res) {
    ...
    try {
      const r = await getSeries({ windowSec, everySec, signal: abortOnClose(res) })
  ...
  async function handleTrack (req, res) {
    ...
    const windowSec = dur(q.get('window'), 3600, 60, 86400)
    try {
      const r = await getTrack({ windowSec, signal: abortOnClose(res) })
      if (!r.ok) return sendJson(res, r.status || 502, { ok: false, code: 'history-error', message: r.message })
  ...
  return { start, stop, status, available, handleSeries, handleTrack, _getSeries: getSeries, _getTrack: getTrack }
}
